...
of the true posterior. First, we can look at $ \hat{R} $ and $ n_{\text{eff}} $. which measures the
level of agreement between our four independent Markov chains. An $ \hat{R} \leq 1.01 $
implies close agreement between the chains. We should also look at $ n_{\text{eff}} $, 
which provides a proxy measure of __effective__ sample size, which we can
think of as sample size adjusted down as a function of how correlated different
samples from the posterior are.
```{r}
...
```{r}
pairs(fit_karin_et_al, pars = pars)
```
