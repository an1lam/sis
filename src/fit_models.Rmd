...
snc_sr_counts_pred <- cbind(
  as.data.frame(
    summary(
      fit_snc_sr_model, pars = "pred_cells_oos_young", 
      probs = c(0.05, 0.5, 0.95)
    )$summary
  ), 
  t = seq(from = 9, to = 13)
...
p_sr <- ggplot(snc_sr_counts_pred, mapping = aes(x = t)) +
  geom_ribbon(aes( ymin = X5., ymax = X95.), fill = "orange", alpha = 0.6) +
  geom_line(mapping = aes(x = t, y = X50.)) +
  scale_x_continuous(name = "Week", breaks=t, labels=t, limits = c(8, 13)) +
  labs(title = "Saturated Removal Model Predictions", x = "Week", y = "SnC (normalized)", color = "name")
p_sr
```
...
```{r}
sr_pars=c('beta', 'sigma', 'lp__', 'pred_cells_means')
...
```{r}
sis_pars <- c('beta', 'll', 'sigma', 'lp__', 'pred_cells_means')
...
snc_sr_sim_counts_pred_young <- cbind(
  as.data.frame(
    summary(
      fit_snc_sr_sim_young, pars = "pred_cells_means", 
...
snc_sr_sim_counts_pred_old <- cbind(
  as.data.frame(
    summary(
      fit_snc_sr_sim_old, pars = "pred_cells_means", 
      probs = c(0.25, 0.5, 0.75)
    )$summary
  ), 
  t_old,
...
snc_sis_sim_counts_pred_young <- cbind(
  as.data.frame(
    summary(
      fit_snc_sis_sim_young, pars = "pred_cells_means", 
      probs = c(0.25, 0.5, 0.75)
    )$summary
  ), 
  t_young,
...
snc_sis_sim_counts_pred_old <- cbind(
  as.data.frame(
    summary(
      fit_snc_sis_sim_old, pars = "pred_cells_means", 
      probs = c(0.25, 0.5, 0.75)
    )$summary
  ), 
  t_old,
